...
function toolbox_content_nav( $nav_id ) {
	...
	<nav id="<?php echo $nav_id; ?>">
		<h1 class="assistive-text section-heading"><?php _e('','toolbox'); ?></h1>
	...
	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>
		...
		<div class="nav-previous"><?php next_posts_link( __( 'Zurück', 'toolbox' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( 'Weiter', 'toolbox' ) ); ?></div>
		<?php endif; ?>
...
function toolbox_posted_on() {
	printf( __('<time class="entry-date" datetime="%1$s" pubdate><em>\'%2$s</em></time>', 'toolbox'), esc_attr(get_the_date('c')), esc_html(get_the_modified_date('y')));
}
endif;
